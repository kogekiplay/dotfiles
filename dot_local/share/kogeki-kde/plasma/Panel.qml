...
Item {
    ...
    readonly property bool invisibleReservePanel: bottomEdge && !floating && panel.thickness >= 68 && panel.thickness <= 96
    readonly property bool macFloatingBottomDock: bottomEdge && floating && panel.thickness >= 80
    readonly property int macFloatingBottomDockVisualThickness: macFloatingBottomDock ? 64 : panel.thickness
    readonly property int macFloatingBottomDockReserveExtra: macFloatingBottomDock ? panel.thickness - macFloatingBottomDockVisualThickness : 0
    ...
    KSvg.FrameSvgItem {
        ...
        imagePath: root.invisibleReservePanel || containment?.plasmoid?.backgroundHints === PlasmaCore.Types.NoBackground ? "" : "widgets/panel-background"
    }
    KSvg.FrameSvgItem {
        ...
        y: root.bottomEdge ? root.fixedTopFloatingPadding + root.macFloatingBottomDockReserveExtra + root.fixedBottomFloatingPadding * (1 - root.floatingness) : root.topFloatingPadding
        width: root.verticalPanel ? panel.thickness : parent.width - root.leftFloatingPadding - root.rightFloatingPadding
        height: root.verticalPanel ? parent.height - root.topFloatingPadding - root.bottomFloatingPadding : root.macFloatingBottomDockVisualThickness
    ...
    KSvg.FrameSvgItem {
        ...
        anchors.fill: floatingTranslucentItem
        imagePath: root.invisibleReservePanel || containment?.plasmoid?.backgroundHints === PlasmaCore.Types.NoBackground ? "" : "solid/widgets/panel-background"
    }
    KSvg.FrameSvgItem {
        id: opaqueItem
    ...
    onStateTriggersChanged: {
        ...
        // bottom dock: keep floating so bottom padding remains visible
        if (bottomEdge && floating) {
            floatingnessTarget = 1
            floatingApplets = true
        }
    ...
    Item {
        ...
        height: root.verticalPanel ? root.height - root.fixedBottomFloatingPadding - root.fixedTopFloatingPadding : root.macFloatingBottomDockVisualThickness
    }
}
